...
class ListViewController: UIViewController,UITableViewDataSource, UITableViewDelegate {
       ...
       @objc func 删除按钮点击(_ notification: Notification) {
           if let cell = notification.object as? ListCell,
              let indexPath = tabList.indexPath(for: cell) {
               // 获取所点击的单元格的标题
               let 删除标题 = ListViewController.List2[indexPath.row].标题
               // 将标题赋值给待删除数据的标题
               待删除数据标题 = 删除标题
      
           
                
               if let index = ListViewController.List2.firstIndex(where: { $0.标题 == 待删除数据标题 }) {
                              // 删除指定索引的元素
                   ListViewController.List2.remove(at: index)
    ...
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        return ListViewController.List2.count
    }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.标题.text = ListViewController.List2[indexPath.row].标题

        let listModel = ListViewController.List2[indexPath.row]
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        let selectedModel = ListViewController.List2[indexPath.row].标题
